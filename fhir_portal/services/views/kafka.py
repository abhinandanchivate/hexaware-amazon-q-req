...
from ..models import KafkaDeadLetterRecord, KafkaEventRecord, KafkaTopicConfiguration
from ..sample_utils import deep_merge, ensure_list, generate_identifier
...
def event_schema(request):
    ...
    template = deep_merge(_base_event_template(), overrides)
    KafkaEventRecord.objects.update_or_create(
...
def configuration(request):
    ...
    for topic_name, config in template['topics'].items():
        KafkaTopicConfiguration.objects.update_or_create(
            name=topic_name,
...
def governance(request):
    ...
    KafkaDeadLetterRecord.objects.update_or_create(
        dlq_event_id=generate_identifier('dlq'),
        defaults={
            'original_event': {},
            'failure_info': retry_policy,
            'routing': template.get('security', {}),
        },
    )
    return Response(template)
